...
public:
    FakeController()
        ...
        publish_timer_ = this->create_wall_timer(20ms, [this] {
            ...
            js.name = jointNames_;
            js.position.resize(jointValues_.size());
            std::ranges::copy(jointValues_, js.position.begin());
...
private:
    ...
    void handle_accepted(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        std::thread{&FakeController::execute_goal, this, goal_handle}.detach();
    }
    ...
    void execute_goal(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        auto apply_positions = [&](const std::vector<double> &positions_incoming) {
            std::vector<double> mine(jointNames_.size(), 0.0);
            for (size_t i = 0; i < positions_incoming.size(); ++i) {
                mine[incoming_to_mine[i]] = positions_incoming[i];
            }
           jointValues_ = mine;
        };
        ...
        while (rclcpp::ok()) {
            ...
            interp.resize(p0.positions.size());
